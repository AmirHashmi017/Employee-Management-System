...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($employee_id) && !empty($task_title) && !empty($priority) && !empty($status)) {
        $stmt = $conn->prepare("INSERT INTO tasks (employee_id, task_title, task_description, priority, status, deadline) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssss", $employee_id, $task_title, $task_description, $priority, $status, $deadline);
